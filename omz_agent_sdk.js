...
function makeRequest(method = "GET", url, token, callback, params = null) {
...
OmnizeVish.prototype = {
  connect: function () {
    ...
    this.channel.join()
      .receive("ok", resp => {
        self.channel.push('get_customers', { accountId: self.accountId });
      })
  ...
  createHandlers: function () {
      ...
      if (pendingRequest) {
        switch (pendingRequest.type) {
          case 'acceptInteraction':
            if (self.events['acceptedInteraction']) self.events['acceptedInteraction'](pendingRequest.interactionHash, data.status, data.reason);
        }
      } else {
        switch (data.status) {
          case 'ok':
            switch (data.response.type) {
              case 'registered':
                if (self.events['registered']) self.events['registered']();
                break;
              case 'unregistered':
                if (self.events['unregistered']) self.events['unregistered']();
                break;
              case 'transferred':
                if (self.events['transferred']) self.events['transferred'](data.response.interactionHash);
                break;
              case 'mailDelivered':
                if (self.events['mailDelivered']) self.events['mailDelivered'](data);
                break;
              case 'receivedMessage':
                if (self.events['receivedMessage']) self.events['receivedMessage'](data.response);
                break;
            };
            break;
          case 'error':
            switch (data.response.reason) {
              case 'unauthorized':
                if (self.events['unauthorized']) self.events['unauthorized']();
                break;
              case 'transferFailed':
                if (self.events['transferFailed']) self.events['transferFailed'](data.response.interactionHash);
                break;
              case 'mailFailed':
                if (self.events['mailFailed']) self.events['mailFailed'](data.response);
                break;
            };
            break;
        }
    ...
    this.channel.on('agent_message', data => {
      ...
      switch (data.type) {
        case 'newMessage':
          if (self.events['newMessage']) self.events['newMessage'](data);
          break;
        case 'receivedMessage':
          if (self.events['receivedMessage']) self.events['receivedMessage'](data);
          break;
        case 'deliveredMessage':
          if (self.events['deliveredMessage']) self.events['deliveredMessage'](data);
          break;
        case 'typing':
          if (self.events['typing']) self.events['typing'](data.interactionHash);
          break;
        case 'cleared':
          if (self.events['cleared']) self.events['cleared'](data.interactionHash);
          break;
        case 'newInteraction':
          if (self.events['newInteraction']) self.events['newInteraction'](data);
          break;
        case 'canceled':
          if (self.events['canceled']) self.events['canceled'](data.interactionHash);
          break;
        case 'finishedInteraction':
          if (self.events['finishedInteraction']) self.events['finishedInteraction'](data.interactionHash);
          break;
        case 'updateInteraction':
          if (self.events['updateInteraction']) self.events['updateInteraction'](data);
          break;
        case 'kill':
          if (self.events['kill']) self.events['kill']();
          break;
      }
    });
  },
  ...
  newMessage: function (message, interactionHash, contentType = 'TEXT') {
    var params = {
    ...
    this.channel.push("newMessage", params);
  ...
  typing: function (interactionHash) {
    this.channel.push("typing", { interactionHash: interactionHash });
  },
  cleared: function (interactionHash) {
    this.channel.push("cleared", { interactionHash: interactionHash });
  },
  acceptInteraction: function (interactionHash) {
    this.pendingRequests.push({
      type: 'acceptInteraction',
      ref: this.channel.push("acceptInteraction", {
        interactionHash: interactionHash
      }).ref,
      interactionHash
    });
  ...
  finishInteraction: function (interactionHash) {
    this.channel.push("finishInteraction", { interactionHash: interactionHash });
  },
  transferToDepartment: function (toDepartmentId, interactionHash) {
    var params = {
      interactionHash: interactionHash,
  ...
  transferToAgent: function (toAgentId, toDepartmentId, interactionHash) {
    var params = {
      interactionHash: interactionHash,
      toAgentId: toAgentId,
      toDepartmentId: toDepartmentId
